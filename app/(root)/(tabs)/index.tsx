...
export default function index() {
  ...
  const refreshing = isFetchingAccounts || isFetchingTransactions || isFetchingBudgets;
  const isLoading = isLoadingAccounts || isLoadingTransactions || isLoadingBudgets;
          ...
          <View className="flex-row gap-2.5">
            {QUICK_ACTIONS.map((action) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push({
                    pathname: "/(root)/(tabs)/addTransaction",
            ...
            <View className="bg-white rounded-[18px] border border-[#E8E6DF] p-4 mb-[18px]">
              ...
              <View className="flex-row items-center">
                ...
                <View className="flex-1 ml-4 gap-1.5">
                  {expenseBreakdown.slice(0, 10).map((c) => (
                    <View
                      key={c.category}
